...
def cornersHeuristic(state, problem):
    ...
    # corner is now the current location, repeat.
    # current_location = state[0]  # Tuple, represents where we are e.g., (25, 12)
    # while unexplored_corners:
    #     corners_and_distances_to_corners = [
    #         (unexplored_corner, mazeDistance(current_location, unexplored_corner, problem.startingGameState))
    #         for unexplored_corner in unexplored_corners
    #     ]
    #
    #     closest_corner, distance_to_closest_corner = min(corners_and_distances_to_corners)
    #     heuristic_total_distance += distance_to_closest_corner
    #
    #     # Move to closest corner (PacMan doesn't actually move here instantly - We
    #     # use this to calculate the next closest corner)
    #     current_location = closest_corner
    #     unexplored_corners.remove(closest_corner)
    #
    # return heuristic_total_distance
...
def foodHeuristic(state, problem):
    ...
    if foodGrid.count() != 0:
        for food in foodGrid.asList():
            heuristic_distance.append(mazeDistance(position, food, problem.startingGameState))
        score = max(max(heuristic_distance), 0)
    return score

    # position, unexplored_corners = state
    # score = 0
    # heuristic = {}
    # foods_copy = list(foodGrid.asList())
    # foodCount = foodGrid.count()

    # if (foodCount!=0):
    #     heuristic = {}
    #     for food in foods_copy:
    #         heuristic[(position, food)] = mazeDistance(position, food, problem.startingGameState)

    #     min_pair = min(heuristic.items(), key=lambda x: x[1])
    #     position = min_pair[0][1]
    #     min_h = min_pair[1] 
    #     score += min_h 
    #     foods_copy.remove(position)
    #     foodCount -= 1
